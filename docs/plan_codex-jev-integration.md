...
両製品の導入とJevによる実操作は成立した。ブラウザは1リンク遷移を通常操作と各5回比較し、両方5/5成功、中央値は通常9.444秒・Jev1.261秒。2リンクの連続操作もJevで5/5成功した。デスクトップは上流の画面取得エラーを避けられる公式0.4.7と無改変の上流Jevスクリプトを使用し、クリックと日本語入力に成功。計算機の反復は3/5成功、成功回の中央値3.238秒で、上流の自信不足による停止率が残る。文字入力は1.24秒で成功した。ブラウザ文字入力用の別モデルのAPIキーは未設定。[ブラウザ評価](evidence/jev-upstream-evaluation-20260921.md)、[デスクトップ評価とMacの呼出し方](evidence/jev-desktop-v047-acceptance-20260921.md)、[利用先](../shared/runbooks/jev-computer-use.md)を参照する。
