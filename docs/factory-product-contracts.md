...
更新日: 2026-08-30。正本はdotagents。host期待状態は [factory-host-product-matrix.md](factory-host-product-matrix.md)、wire契約はServerManager `bughub/FACTORY_INTEGRATION.md`。
...
- 初回導入と再適用はMacの`setup-macos-factory.sh`、main-serverの`setup-linux-factory.sh`、rabbitの`setup-linux-workstation-factory.sh`、Windows nativeの`setup-windows-native-factory.ps1`が所有する。4入口はdeployment contractの製品集合だけを共有し、OS固有のLaunchAgent／cron／Task Scheduler、config、hook、credential、delivery receiptを混同しない。Linux 2入口は共通本体を使うが`server`／`linux` profileの役割を混同しない。各入口は定期更新を冪等登録し、`verify-install`、[工場の現行状態](factory-current-state.md)が示すwireのfresh report、BugHub delivery receiptまでを受け入れる。
- Windows nativeの工場shellはPowerShell 7（`pwsh.exe`）だけとし、一撃入口・定期Task・Windows adapterは同じ実体を使う。5.1しかないhostはMicrosoft公式installer／package managerで7を導入してから再実行し、5.1／`cmd.exe`へfallbackしない。POSIX script用の`bash.exe`／`sh.exe`はGit for Windowsのnative executableに限定する。WSL2・Docker・Hyper-V・Virtual Machine PlatformはWindows nativeの製品依存・導入前提・fallbackではなく、一撃入口も状態を変更／検証しない。
- Windows nativeからmain-serverへの管理SSHも同入口のhost固有契約である。専用ED25519鍵はpassphraseなし・owner-only ACL、server host keyはfingerprint固定、configは`main-server`と`192.168.1.2`の双方で`IdentitiesOnly yes`とする。接続不能ならGitHubへ公開鍵だけを設定し、main-server self-hosted runnerの登録workflowで制限付き`authorized_keys`行をexactly-once登録する。alias 3回＋直IP 1回の`BatchMode`接続が成功しない限りfail closedにする。
- main-serverからrabbitへの管理SSHはrabbit一撃入口のhost固有契約である。rabbit root phaseはUbuntu公式OpenSSH Server、公開鍵認証、password login禁止、root login禁止、`kite`のpasswordless sudoersを冪等管理する。main-serverにはpassphraseなしのrabbit専用ED25519鍵、固定済みrabbit host key、`rabbit`／`192.168.1.55`の`IdentitiesOnly yes`設定を配線し、rabbitの`authorized_keys`は同公開鍵へagent／port／X11 forwarding禁止を付けてexactly-once登録する。alias／直IPの`BatchMode`接続とremote `sudo -n`が成功しない限りfail closedにする。
