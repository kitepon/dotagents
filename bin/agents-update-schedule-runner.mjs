...
if (platform() !== 'win32' || process.argv.length !== 2) fail('Windows nativeのscheduled taskだけが実行できます');
  ...
  const state = join(local, 'dotagents', 'factory-reporter-v9');
  if (!existsSync(runner) || !existsSync(bash)) fail('scheduled runner preflightが失敗しました（install.sh配布またはGit Bashがありません）');
    ...
    const result = spawnSync(bash, [runner], { encoding: 'utf8', env: { ...process.env, HOME: home, CODEX_HOME: join(home, '.codex'), AGENTS_UPDATE_PATH_PREFIX: `${msysHome}/.local/bin`, AGENTS_UPDATE_BATCH_TOKEN: token, FACTORY_REPORTER_RUNNER: join(home, '.local', 'bin', 'factory-reporter-v9-schedule-runner') } });
    if (result.status !== 0) fail(`agents-updateがexit ${result.status}で失敗しました`);
      ...
      else try { const report = JSON.parse(readFileSync(join(state, 'latest-report.json'), 'utf8')); const receipt = JSON.parse(readFileSync(join(state, 'delivery-receipt.json'), 'utf8')); assertDeliveryReceipt({ report, priorReportId: beforeReport, receipt, batchToken: token }); process.stdout.write(`${JSON.stringify({ ok: true, batch_token: token, report: 'v9', delivery_acknowledged: true })}\n`); } catch { fail('今回のv9 reportとBugHub accepted後のdelivery receiptを確認できません'); }
    }
  }
}
