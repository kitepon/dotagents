...
- 残Hの受入は **Cmd+Shift+L の新規ローカル Agent チャット**で 2026-08-24 20:06 JST に閉じた（conversation `d4c055fa-a8a9-4a71-b2d1-88901e6fbfd8`）。`bc-7face712` の follow-up と goal continuation は数えない。次も数えない: Task/generalPurpose（`bc-` cloud agent）、`cursor --chat`（workspaceOpen のみ。x-article 窓が数分おきに再利用される）、未login の `agent -p`、macOS `open` と CLI `--open-url` の `cursor://` prompt、osascript keystroke、desktopBridge feature gate。証拠は sessionStart の `is_background_agent: false`、Desktop hook ログの `Executing hook … from user config`、`constitution-delivered/d4c055fa-…` stamp、モデルが cap 内案内を受けて `~/.cursor/rules/factory.mdc` を Read したこと。
...
Mac新規session受入（2026-08-24 20:06 JST）。conversation `d4c055fa-a8a9-4a71-b2d1-88901e6fbfd8`。sessionStart payload は `is_background_agent: false` / `composer_mode: agent`。dotagents workbench（workspace `7aedc4b5…`）が 11:06:00Z に sessionStart 10本・beforeSubmitPrompt 8本を実行し、先頭は `from user config` の `/Users/kite/.local/bin/cursor-constitution-hook`（exit 0）。stamp `~/.cursor/factory-hook-state/constitution-delivered/d4c055fa-a8a9-4a71-b2d1-88901e6fbfd8`（mtime 20:06:00、内容 `1`）。Desktop は additional_context 10189 字を 10000 cap 超過として agent-tools へ spill。モデルは cap 内案内（ベル・Cursor native shell・正本パス）を受け、`~/.cursor/rules/factory.mdc` を Read。本文 10750 字、人格ベル、`Cursor native` あり、Claude 固有の日常shell既定と `## Claude固有` は無し。残Hは閉じた。
