...
export const V2_PRODUCT_IDS = Object.freeze(['caveat', 'throughline', 'spotter', 'codegraph', 'markitdown', 'gpt-connector', 'aiterm-mcp', 'codex-sidecar', 'servermanager', 'claude-code', 'codex-cli', 'grok-build']);
...
function empty() { return { presence_status: 'unverified', contract_version: '2.0', checks: [], runtime_errors: [], resolutions: [] }; }
...
function throughlineDiagnostic(diagnostic) {
  ...
  const acceptedDatabaseSchemaTuple = (diagnostic.databaseSchema.schema === 'throughline.database.v9' && diagnostic.databaseSchema.supportedDatabaseSchemaVersion === 9 && (diagnostic.databaseSchema.databaseSchemaVersion === 9 || (diagnostic.databaseSchema.databaseSchemaVersion === null && diagnostic.databaseSchema.status === 'not_applicable'))) || (diagnostic.databaseSchema.schema === 'throughline.database.v8' && ((diagnostic.databaseSchema.databaseSchemaVersion === null && diagnostic.databaseSchema.supportedDatabaseSchemaVersion === 8) || (diagnostic.databaseSchema.databaseSchemaVersion === 8 && diagnostic.databaseSchema.supportedDatabaseSchemaVersion === 8) || (diagnostic.databaseSchema.databaseSchemaVersion === 9 && diagnostic.databaseSchema.supportedDatabaseSchemaVersion === 9)));
...
function nativeResult(product, id, diagnostic, result, now) {
  try {
    ...
    if (id === 'caveat') { const parsed = parseCaveatDiagnostic(diagnostic); ({ overall, state, migration } = parsed); installedVersion = parsed.version; }
...
async function markitdown(cwd) { const result = await run('markitdown', ['--version'], { cwd }); const installed = result.ok ? version(result.stdout) : null; if (!installed) return unavailable(result, 'version'); const dir = await mkdtemp(join(tmpdir(), 'factory-v2-markitdown-')); try { const fixture = join(dir, 'fixture.txt'); await writeFile(fixture, 'factory fixture\n'); const converted = await run('markitdown', [fixture], { cwd }); return { ...empty(), presence_status: 'installed', installed_version: installed, checks: [check('local_fixture', converted.ok && Buffer.byteLength(converted.stdout) > 0 ? 'pass' : 'unverified', converted.ok ? undefined : 'fixture_failed')] }; } finally { await rm(dir, { recursive: true, force: true }); } }
...
function canonicalCodexHookCommand(command, home) {
  const hookPath = join(home, '.local', 'bin', 'codex-git-destroy-gate-hook');
  if (process.platform !== 'win32') {
    return command === ['/usr/bin/env', 'python3', hookPath].map(posixShellQuote).join(' ');
  }
...
async function claudeIntegration(home, now) {
  try {
    ...
    const required = {
      PreToolUse: { matcher: 'Bash', hook: { type: 'command', command: '~/.local/bin/git-destroy-gate-hook', timeout: 5 } },
    };
...
async function codexIntegration(home, cwd, now) {
  ...
  try {
    ...
    const required = { PreToolUse: 5 };
    for (const [event, timeout] of Object.entries(required)) {
      const entries = Array.isArray(value?.hooks?.[event]) ? value.hooks[event] : [];
      const candidates = entries.flatMap((entry) => Array.isArray(entry?.hooks) ? entry.hooks : []).filter((item) => canonicalCodexHookCommand(item?.command, home));
      const canonical = entries.filter((entry) => {
        ...
        return hook && typeof hook === 'object' && !Array.isArray(hook)
          ...
          && hook.type === 'command'
          && canonicalCodexHookCommand(hook.command, home)
...
export async function scanV2WithAcknowledgements({ host, cwd, arch, platform, collectionEnabled = false, toolchainLedgerPath }) {
  ...
  const now = new Date().toISOString(); const products = Object.fromEntries(V2_PRODUCT_IDS.map((id) => [id, empty()]));
  ...
  products.servermanager = host.profile === 'server' ? await serverManagerNative({ cwd }, now) : { ...empty(), presence_status: 'not_applicable' };
  ...
  const report = { schema_version: '2.0', report_id: randomUUID(), host_id: host.id, host_profile: host.profile, platform: { os, arch }, report_mode: 'full', observed_at: observed, created_at: created < observed ? observed : created, reporter: { version: '2.0.0', dotagents_revision: revision.stdout.trim() }, products }; return { report, acknowledgements: { schema_version: '2.0', report_id: report.report_id, acknowledgements: [...(gptResult.acknowledgement ? [gptResult.acknowledgement] : []), ...(serverManagerAcknowledgement ? [serverManagerAcknowledgement] : [])] } };
}
export async function scanV2(options) { return (await scanV2WithAcknowledgements(options)).report; }
