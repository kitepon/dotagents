...
function peertableDiagnostic(value) {
  if (!semver(value?.product?.version) || !['ready', 'not_ready', 'unverified'].includes(value.overall)
    || !value.checks || typeof value.checks !== 'object' || Array.isArray(value.checks)
    || !Object.entries(value.checks).every(([id, status]) =>
      /^[a-z][a-z0-9_]{0,63}$/.test(id) && ['pass', 'fail', 'not_applicable', 'unverified'].includes(status))) {
...
function peertableChecks(diagnostic, now) {
  return Object.entries(diagnostic.checks).map(([id, status]) => {
