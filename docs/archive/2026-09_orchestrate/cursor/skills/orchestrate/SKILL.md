---
name: orchestrate
description: 統括レーン（①計画に中断が組込済み②受入が多段連鎖③複数repoの書込調整④裁定証跡が必要、のいずれか確定）の実装、監査、移行を Cursor 親のnative・外部実行・相談レーンで安全に統括する時に使う。技法は通常レーンでも参照可、Control儀式は統括レーンだけ。
---
...
- **円卓の入口は peertable room だけ**（`peertable` skill。Windows native も 2026-08-16 修理後に利用可）。campaign またはオーナーが円卓を指定した実装物量は、`Task` へ流さない。Cursor 親は工場MCPに room を持たないので HTTP API で着卓し、席は `launch-seat.sh` で立てる。
...
固定Recipe `adversarial-audit`／`bulk-curation` のPhase・入出力schema・reducer・gate・失敗条件の正本は[固定Recipe契約](references/shared-orchestrate/recipes.md)。本節はCursor実行入口だけを所有する。

- **fan-out**: 各視点・各指摘・各対象を1子1任務として`Task`へ出す。read-only段は並列してよい。同一repoへ書込む対象が2つ以上ありLatticeが選択されていなければ直列（正本は[合成契約](references/shared-orchestrate/composition.md)）。
