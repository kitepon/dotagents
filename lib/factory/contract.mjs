...
const V9_PRODUCT_IDS = [...V8_PRODUCT_IDS, 'jev-ultrafast', 'agent-desktop'];
...
const SAFE_CONTEXT_ALLOWLIST = Object.freeze(
  Object.fromEntries([...new Set([...PRODUCT_IDS, ...V2_PRODUCT_IDS, ...V4_PRODUCT_IDS, ...V5_PRODUCT_IDS, ...V6_PRODUCT_IDS, ...V7_PRODUCT_IDS, ...V8_PRODUCT_IDS, ...V9_PRODUCT_IDS])].map((id) => [id, new Set()])),
);
...
function validateRuntimeError(item, path, observedAt, productId) {
  exactKeys(item, [
    ...
    'occurrence_count', 'first_seen', 'last_seen', 'state_schema_version', 'safe_context', 'product_version',
  ], path);
  ...
  if ('product_version' in item) version(item.product_version, `${path}.product_version`);
  if ('safe_context' in item) safeContext(item.safe_context, `${path}.safe_context`, productId);
...
export function validateReportV9(report) {
  validateReportVersion(report, '9.0', V9_PRODUCT_IDS);
}
...
export async function readAndValidateReportV9(reportPath) {
  const bytes = await readFile(reportPath);
  let report;
  try {
    report = JSON.parse(UTF8.decode(bytes));
