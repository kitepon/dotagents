...
- 各製品は自身のソース・状態・schema・migration・正規診断を所有する。製品で得た知識は、その製品が未導入でも成立する判断と理由へ一般化して工場へ還流させる。製品固有の所属・専用ツール・実行環境を工場の共通規範の前提へ持ち込まない。dotagentsはそれらを複製せず統合契約を所有し、ServerManager/BugHubはdotagentsの代わりに工場方針を決めたり製品状態を直接書き換えたりしない。
- **製品の不備を工場で補完することは禁止**。dotagentsが製品の導入・設定・更新に使うのは、製品が提供する標準的なinstaller・package manager・公開コマンドと、その公開結果だけとする。呼出し方法も製品とOSの標準に従う。製品に必要な設定・hook・MCP登録・移行・OS差の吸収は製品側が所有する。正規入口の不足や失敗を、工場側の直接編集・内部手順の組立て・独自wrapperで補うと単体の不具合を隠すため、それらは製品側の修理対象とする。失敗・未対応はそのまま可視化し、成功として扱わない。dotagents自身の規範・設定・hook・配布・更新予約・工場報告はdotagentsが所有する。
...
3. **趣旨・原則・残件は [PLAN.md](PLAN.md)（憲章＝聖典 v4）が正**。環境まわりの作業はまず PLAN.md で現在地を拾い、Latticeの明示適用が決まった作業またはオーナー承認済みの既存Lattice工程を継続する時だけ、その現行状態も確認する。判断に迷ったら原則に立ち返る。文書の5分類・archive・所有境界は [docs/00_overview.md](docs/00_overview.md) と [document registry](docs/document-registry.json) が正。製品内部の導入・設定・状態・schema・migration・診断・復旧・更新・releaseをdotagentsへ正本化しない。
