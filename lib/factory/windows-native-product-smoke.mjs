...
function passOrFreshNotApplicable(product, id, checkId) {
  const item = Array.isArray(product.checks) ? product.checks.find((candidate) => candidate?.check_id === checkId) : null;
  if (!item || (item.status !== 'pass' && !(item.status === 'skipped' && item.reason_code === 'not_applicable'))) {
    throw new Error(`${id}:${checkId} is neither pass nor fresh not_applicable`);
  }
}
...
export function assertWindowsNativeProductSmoke(report, runtimeArch = process.arch) {
  ...
  for (const [id, checks] of Object.entries(CORE_CHECKS)) {
    ...
    for (const checkId of checks) {
      if (id === 'throughline' && ['database_schema', 'restore'].includes(checkId)) passOrFreshNotApplicable(product, id, checkId);
      else pass(product, id, checkId);
    }
  }
