...
- 残Hの受入は、人が Desktop で Cmd+Shift+L（Open New Agent Chat）し、最初の文を送ったチャットだけ。次は Desktop hook を踏まないので数えない: この既存sessionと goal continuation（private worker。beforeSubmitPrompt / preToolUse がログに出ない）、Task/generalPurpose（`bc-` cloud agent。Desktop `hooks.json` を走らせない）、`cursor --chat`（workspaceOpen のみ。x-article 窓が数分おきに再利用される）、未login の `agent -p`、deeplink / osascript keystroke / desktopBridge feature gate。hook 実行の証拠は `cursor-constitution-hook` が Desktop hook ログに出ることと、`~/.cursor/factory-hook-state/constitution-delivered/` に stamp が付くこと。
...
- **H:** 実HOMEの`install.sh` / `apply-cursor-config --apply` は済。Wave 6の製品repoは Throughline 0.10.3 まで完了。Macの工場MCPは apply 後の Desktop 別project で tool 列挙、空workspaceの `agent mcp list` で工場6が `ready`。Throughline は `cursor:` session を DB に書いた。残Hは人が Cmd+Shift+L した新規 Desktop Agent チャットで、`cursor-constitution-hook` が beforeSubmitPrompt を走り、stamp が付き、モデルが cap 内案内を受けて `factory.mdc` を Read すること。工場skillは `~/.cursor/skills` から列挙（入れ子 Task でも 4 本は見えた。Claude skill の併記は切断しない）。この既存session・Task cloud・`cursor --chat` は数えない。
