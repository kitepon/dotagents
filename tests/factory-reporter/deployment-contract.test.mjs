...
test('deployment contractは管理14製品とv9 wire 17 IDを固定する', () => {
  assert.deepEqual(MANAGED_PRODUCT_IDS, [
    'caveat', 'throughline', 'spotter', 'lattice', 'markitdown', 'gpt-connector',
    'aiterm-mcp', 'codex-sidecar', 'aishell', 'servermanager', 'peertable', 'unai', 'jev-ultrafast', 'agent-desktop',
  ]);
  ...
  assert.deepEqual(hostProjection({ profile: 'mac', os: 'darwin', arch: 'arm64', macosMajor: 15 }).required, ['caveat', 'throughline', 'spotter', 'lattice', 'markitdown', 'gpt-connector', 'aiterm-mcp', 'codex-sidecar', 'aishell', 'peertable', 'unai', 'jev-ultrafast', 'agent-desktop']);
  assert.deepEqual(hostProjection({ profile: 'server', os: 'linux', arch: 'arm64' }).required, ['caveat', 'throughline', 'spotter', 'lattice', 'markitdown', 'gpt-connector', 'aiterm-mcp', 'codex-sidecar', 'servermanager', 'peertable', 'unai', 'jev-ultrafast']);
  assert.deepEqual(hostProjection({ profile: 'linux', os: 'linux', arch: 'x64' }).required, ['caveat', 'throughline', 'spotter', 'lattice', 'markitdown', 'gpt-connector', 'aiterm-mcp', 'codex-sidecar', 'peertable', 'unai', 'jev-ultrafast']);
});
...
test('host別required集合はmatrix全行と完全一致する', () => {
  ...
  assert.deepEqual(required({ profile: 'mac', os: 'darwin', arch: 'arm64', macosMajor: 15 }), ['caveat','throughline','spotter','lattice','markitdown','gpt-connector','aiterm-mcp','codex-sidecar','aishell','peertable','unai', 'jev-ultrafast', 'agent-desktop']);
  assert.deepEqual(required({ profile: 'mac', os: 'darwin', arch: 'arm64', macosMajor: 14 }), ['caveat','throughline','spotter','lattice','markitdown','gpt-connector','aiterm-mcp','codex-sidecar','peertable','unai', 'jev-ultrafast', 'agent-desktop']);
  assert.deepEqual(required({ profile: 'server', os: 'linux', arch: 'arm64' }), ['caveat','throughline','spotter','lattice','markitdown','gpt-connector','aiterm-mcp','codex-sidecar','servermanager','peertable','unai', 'jev-ultrafast']);
  assert.deepEqual(required({ profile: 'linux', os: 'linux', arch: 'x64' }), ['caveat','throughline','spotter','lattice','markitdown','gpt-connector','aiterm-mcp','codex-sidecar','peertable','unai', 'jev-ultrafast']);
  assert.throws(() => required({ profile: 'wsl', os: 'linux', arch: 'x64' }), /profile/u);
  assert.deepEqual(required({ profile: 'windows-native', os: 'win32', arch: 'x64' }), ['caveat','throughline','spotter','lattice','markitdown','gpt-connector','aiterm-mcp','codex-sidecar','peertable','unai', 'jev-ultrafast']);
});
...
test('post-update gateは対応hostのAIShell・peertable・unai欠落を拒否し、非対応を要求しない', () => {
  ...
  const windows = { products: Object.fromEntries(['caveat', 'throughline', 'spotter', 'lattice', 'markitdown', 'gpt-connector', 'aiterm-mcp', 'codex-sidecar', 'peertable', 'unai', 'jev-ultrafast'].map((id) => [id, { presence_status: 'installed', compatibility_status: 'compatible', checks: [] }])) };
