...
const EFFORT_TOKEN = "none|low|medium|high|xhigh|max|ultra";
// 「Label×effort」「Label×low（字面）〜medium（統合）」を1つの順位項目として読む。
const RANK_ENTRY_RE = new RegExp(`([A-Za-z0-9][A-Za-z0-9. ]*?)×(${EFFORT_TOKEN})(?:（[^）]*）)?(?:〜(${EFFORT_TOKEN}))?`, "g");
// 統括は親の役割であり、推薦入口は親を変えない。
const PARENT_ROLE = "統括";

function rankEntriesInCell(cell) {
  const entries = [];
  for (const match of cell.matchAll(RANK_ENTRY_RE)) {
    const family = FAMILY_LABELS[match[1].trim()];
    if (!family) fail("RANK_LABEL_UNKNOWN", `rank label ${match[1].trim()} is not in the recommendation source`);
    entries.push({ family, efforts: [match[2], match[3]].filter(Boolean) });
  }
  // effort を持たない順位: Haiku（effortなし）と ChatGPT（gpt-connector）。
  if (entries.length === 0) {
    if (rankLabelPattern("Haiku").test(cell)) entries.push({ family: FAMILY_LABELS.Haiku, efforts: [null] });
    else if (rankLabelPattern("ChatGPT").test(cell)) entries.push({ family: FAMILY_LABELS.ChatGPT, efforts: [null] });
  }
  return entries;
}

// 02 の順位表を「役割 → 順位付き (family, efforts[])」へ読む。順位表の文言は 02 だけが持つ
// (ADR 20260922) ので、推薦入口は実行時にここから読み、複製を持たない。
export function parseRankTable(markdown) {
  const rows = rankSection(markdown).split("\n").filter((line) => line.startsWith("|")).slice(2);
  const roles = [];
  for (const row of rows) {
    const cells = row.split("|").slice(1, -1).map((entry) => entry.trim());
    const role = cells[0].replace(/\*\*/g, "");
    if (role === PARENT_ROLE) continue;
    const ranks = [];
    for (const cell of cells.slice(1)) {
      if (!/[A-Za-z0-9]/.test(cell)) continue;
      const entries = rankEntriesInCell(cell);
      // 「Sol/Terraで独立確認」のような順位でない散文セルは順位に数えない。
      if (entries.length === 1) ranks.push(entries[0]);
    }
    if (ranks.length === 0) fail("RANK_TABLE_MISSING", `role ${role} has no rank entry`);
    roles.push({ role, ranks });
  }
  if (roles.length === 0) fail("RANK_TABLE_MISSING", "rank table has no role rows");
  return roles;
}
