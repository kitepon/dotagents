...
- 全documentは[document registry](docs/document-registry.json)が先頭一致で`generated`／`current`／`contract`／`history`／`evidence`へ分類する。変動する現行値は`lib/factory/deployment-contract.mjs`から生成し、current文書へ手入力しない。`make lint-current-docs`は生成driftと分類逃れを拒否する。
