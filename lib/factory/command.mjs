...
function environmentValue(env, name) {
  // 呼出側が PATH のような正規名で明示上書きした場合は、Windows 由来の
  // Path/PathExt が object 内に残っていてもその指定を優先する。
...
function appContainerRedirectedEntrypoint(shimDirectory, lexicalRelative, resolved, pathModule) {
  if (pathModule.basename(shimDirectory).toLowerCase() !== 'npm') return null;
  const roaming = pathModule.dirname(shimDirectory);
  if (pathModule.basename(roaming).toLowerCase() !== 'roaming') return null;
  const packages = pathModule.join(pathModule.dirname(roaming), 'Local', 'Packages');
  const redirectedRelative = pathModule.relative(packages, resolved);
  if (!redirectedRelative || redirectedRelative === '..' || redirectedRelative.startsWith(`..${pathModule.sep}`) || pathModule.isAbsolute(redirectedRelative)) return null;
  const segments = redirectedRelative.split(pathModule.sep);
  if (segments.length < 6 || !/^OpenAI\.Codex_[A-Za-z0-9]+$/iu.test(segments[0])
    || segments[1].toLowerCase() !== 'localcache' || segments[2].toLowerCase() !== 'roaming'
    || segments[3].toLowerCase() !== 'npm' || segments[4].toLowerCase() !== 'node_modules') return null;
  if (segments.slice(5).join(pathModule.sep).toLowerCase() !== lexicalRelative.toLowerCase()) return null;
  return pathModule.join(packages, ...segments.slice(0, 4));
}

async function verifiedNpmEntrypoint(shim, relativeEntrypoint, fs, pathModule) {
  const segments = relativeEntrypoint.split('\\');
  if (segments.some((segment) => !segment || segment === '.' || segment === '..')) throw commandError('Windows command shimのentrypointが不正です');
  const shimDirectory = pathModule.dirname(shim);
  const resolvedShimDirectory = await fs.realpath(shimDirectory);
  const nodeModules = pathModule.join(resolvedShimDirectory, 'node_modules');
  const entrypoint = pathModule.join(shimDirectory, ...segments);
  let resolved;
  try {
    if (!await regularFile(entrypoint, fs)) throw commandError('Windows command shimのentrypointが不正です');
    resolved = await fs.realpath(entrypoint);
  } catch (error) {
    if (error?.code === 'EINVAL') throw error;
    throw commandError('Windows command shimのentrypointが不正です', error?.code || 'EINVAL');
  }
  const lexicalNodeModules = pathModule.join(shimDirectory, 'node_modules');
  const lexicalRelative = pathModule.relative(lexicalNodeModules, entrypoint);
  if (!lexicalRelative || lexicalRelative === '..' || lexicalRelative.startsWith(`..${pathModule.sep}`) || pathModule.isAbsolute(lexicalRelative)) throw commandError('Windows command shimのentrypointが不正です');
  const relative = pathModule.relative(nodeModules, resolved);
  const directlyContained = relative && relative !== '..' && !relative.startsWith(`..${pathModule.sep}`) && !pathModule.isAbsolute(relative);
  const pathPrefix = directlyContained ? null : appContainerRedirectedEntrypoint(shimDirectory, lexicalRelative, resolved, pathModule);
  if (!directlyContained && !pathPrefix) throw commandError('Windows command shimのentrypointが不正です');
  return { entrypoint, pathPrefix };
}

async function npmCmdResolution(shim, fs, pathModule) {
  let source;
  try {
  ...
  if (match) {
    const { entrypoint, pathPrefix } = await verifiedNpmEntrypoint(shim, match[1], fs, pathModule);
    return { command: process.execPath, prefixArgs: [entrypoint], ...(pathPrefix ? { pathPrefix } : {}) };
  }

  // npm 11 / Claude Code native distribution が生成する shell 非介在の固定形。
  // node_modules 内の regular .exe だけを実体 containment 検証後に直接起動する。
  const nativePattern = /^@ECHO off\nGOTO start\n:find_dp0\nSET dp0=%~dp0\nEXIT \/b\n:start\nSETLOCAL\nCALL :find_dp0\n+"%dp0%\\(node_modules(?:\\[A-Za-z0-9@._-]+)+\.exe)"\s+%\*\n?$/iu;
  const nativeMatch = nativePattern.exec(source);
  if (!nativeMatch) throw commandError('Windows command shimのentrypointが不正です');
  const { entrypoint } = await verifiedNpmEntrypoint(shim, nativeMatch[1], fs, pathModule);
  return { command: entrypoint, prefixArgs: [] };
}
...
export async function resolveWindowsCommand(command, {
  ...
  for (const directory of directories) {
    for (const extension of extensions) {
      ...
      if (extension === '.cmd') return npmCmdResolution(candidate, fs, pathModule);
    }
  }
...
function validAppContainerPathPrefix(entrypoint, pathPrefix, pathModule) {
  if (typeof pathPrefix !== 'string' || !pathModule.isAbsolute(pathPrefix)) return false;
  const entryNodeModules = entrypoint.toLowerCase().lastIndexOf(`${pathModule.sep}node_modules${pathModule.sep}`.toLowerCase());
  if (entryNodeModules < 0) return false;
  const lexicalNpm = entrypoint.slice(0, entryNodeModules);
  if (pathModule.basename(lexicalNpm).toLowerCase() !== 'npm') return false;
  const roaming = pathModule.dirname(lexicalNpm);
  if (pathModule.basename(roaming).toLowerCase() !== 'roaming') return false;
  const packages = pathModule.join(pathModule.dirname(roaming), 'Local', 'Packages');
  const relative = pathModule.relative(packages, pathPrefix);
  if (!relative || relative === '..' || relative.startsWith(`..${pathModule.sep}`) || pathModule.isAbsolute(relative)) return false;
  const segments = relative.split(pathModule.sep);
  return segments.length === 4 && /^OpenAI\.Codex_[A-Za-z0-9]+$/iu.test(segments[0])
    && segments[1].toLowerCase() === 'localcache' && segments[2].toLowerCase() === 'roaming'
    && segments[3].toLowerCase() === 'npm';
}
function validHelperResolution(value, pathModule) {
  const keys = Object.hasOwn(value ?? {}, 'pathPrefix') ? ['command', 'prefixArgs', 'pathPrefix'] : ['command', 'prefixArgs'];
  if (!exact(value, keys) || typeof value.command !== 'string' || !Array.isArray(value.prefixArgs) || value.prefixArgs.some((item) => typeof item !== 'string' || /[\0\r\n]/u.test(item)) || /[\0\r\n]/u.test(value.command) || !pathModule.isAbsolute(value.command)) throw helperError();
  ...
  if (value.prefixArgs.length === 1 && value.command === process.execPath && pathModule.isAbsolute(entrypoint) && /\.(?:cjs|mjs|js)$/iu.test(entrypoint) && segments.includes('node_modules') && !segments.includes('.') && !segments.includes('..')
    && (!Object.hasOwn(value, 'pathPrefix') || validAppContainerPathPrefix(entrypoint, value.pathPrefix, pathModule))) return value;
  throw helperError();
}
function helperResult(value, pathModule) {
  ...
  if (value.status === 'ok' && (exact(value, ['status', 'command', 'prefixArgs']) || exact(value, ['status', 'command', 'prefixArgs', 'pathPrefix']))) return validHelperResolution({ command: value.command, prefixArgs: value.prefixArgs, ...(Object.hasOwn(value, 'pathPrefix') ? { pathPrefix: value.pathPrefix } : {}) }, pathModule);
  ...
  return new Promise((resolve) => {
    ...
    const start = async () => {
      ...
      try {
        let childEnv = env;
        if (pathPrefix) {
          childEnv = Object.fromEntries(Object.entries(env).filter(([key]) => key.toLowerCase() !== 'path'));
          childEnv.PATH = `${pathPrefix}${windowsPathModule.delimiter}${environmentValue(env, 'PATH')}`;
        }
        child = spawn(executable, commandArgs, {
          cwd, env: childEnv, detached: platform !== 'win32',
