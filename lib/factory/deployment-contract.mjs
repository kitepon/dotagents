...
export const THIRD_PARTY_PRODUCT_IDS = Object.freeze(['markitdown', 'jev-ultrafast', 'agent-desktop']);
export const CORE_PRODUCT_IDS = Object.freeze(
  MANAGED_PRODUCT_IDS.filter((id) => !THIRD_PARTY_PRODUCT_IDS.includes(id)),
);
export const CURRENT_WIRE_MAJOR = 9;
export const CURRENT_WIRE_SCHEMA_VERSION = '9.0';
export const CURRENT_WIRE_ENDPOINT = `/api/factory/v${CURRENT_WIRE_MAJOR}/reports`;
export const ROLLBACK_WIRE_MAJOR = 8;
// wire v9の固定製品集合。codex-sidecarは管理対象から外したが、v9では対象外として報告を続ける。
export const CURRENT_WIRE_PRODUCT_IDS = Object.freeze([
  'caveat', 'throughline', 'spotter', 'lattice', 'markitdown', 'gpt-connector', 'aiterm-mcp', 'codex-sidecar',
  'servermanager', 'claude-code', 'codex-cli', 'grok-build', 'aishell', 'peertable', 'unai', 'jev-ultrafast', 'agent-desktop',
]);
export const FACTORY_RUNNERS = Object.freeze([
  Object.freeze({ name: 'factory-macos-m5', label: 'macos-native', fullCi: true }),
  Object.freeze({ name: 'factory-linux-main', label: 'linux-server', fullCi: false }),
  Object.freeze({ name: 'factory-linux-rabbit', label: 'linux-workstation', fullCi: true }),
  Object.freeze({ name: 'factory-windows-fox', label: 'windows-native', fullCi: true }),
]);
export const BASE_NPM_PACKAGES = Object.freeze([
  '@anthropic-ai/claude-code', '@openai/codex', 'gpt-connector', '@anthropic-ai/sdk',
  'aiterm-mcp', 'caveat-cli', 'claude-spotter', '@quolu/lattice', 'peertable', 'pnpm', 'throughline',
]);

const BASE_REQUIRED = Object.freeze(['caveat', 'throughline', 'spotter', 'lattice', 'markitdown', 'gpt-connector', 'aiterm-mcp', 'peertable', 'unai', 'jev-ultrafast']);
// `wsl` は旧wire/configの読取り互換には残すが、現役配備profileではない。
const PROFILES = new Set(['mac', 'server', 'linux', 'windows-native']);
const TOOLCHAINS = Object.freeze(['claude-code', 'codex-cli']);
const PROFILE_OS = Object.freeze({ mac: 'darwin', server: 'linux', linux: 'linux', 'windows-native': 'win32' });
...
export const ALLOWED_UNVERIFIED = new Set([
  ...
  'throughline\0handoff\0diagnostic_unverified',
...
export function hostProjection({ profile, os, arch, macosMajor = null }) {
  ...
  expected['agent-desktop'] = profile === 'mac' ? 'required' : 'unsupported';
