...
- 既存Cursor sessionの見た目（apply前のMCP catalog等）は受入に数えない。`hooks.json` は Desktop **workbench** が live reload する（この窓は 13 user hooks を load 済み）。goal continuation は composer の beforeSubmitPrompt を踏まない。reload済み窓でも、人が送った次の文は workbench の `executeHookForStep(beforeSubmitPrompt)` を踏む（composer 経路。`from user config` はここ）。private worker の exec-daemon は起動時に `~/.cursor/hooks.json` を一度だけ読む（`hasUserHooks`）。この会話の worker は hook 導入前（11:46）に `hasUserHooks: false` で凍結しており、tool の preToolUse は factory hook を走らせない。Cmd+Shift+L の新規は新しい worker なので、今ある `hooks.json` を読む。
...
- Cursor 3.17.8 Desktop の `getGlobalRules` は workspace 内の alwaysApply だけを注入する。`~/.cursor/rules/*.mdc` は Customize に出ても Agent の always-apply には乗らない。`toCursorRule` は `alwaysApply: true` を type=global にし YAML `globs` を捨てる。`getRulesForFiles` は alwaysApply かつ globs 空を除外する。home mdc へ `globs` を足しても本文は乗らない。alwaysApply を外すと overlay の getGlobalRules も死ぬ。`getExplicitRules` の requireDescription は cache の `description` を見るが、global 型は YAML description を copy しない。`$bf` の always_applied 注入は会話の最初の human bubble だけ。User Rules UI 手貼りを完成形にせず、工場 `cursor-constitution-hook` が配達する。`additional_context` は 10000 字まで inline。超過は Desktop が agent-tools の uuid.txt へ spill し、モデルへは「そのファイルを Read」だけが残る（末尾の Cursor delta が落ちる）。現行の生成憲法は cap を超える（共通本文だけでも 10000 超）ため、hook は cap 内の案内（ベル・Cursor native shell・正本パス）と本文冒頭を inline し、末尾の Cursor delta は切らず、同一全文は `factory.mdc` の Read で届ける。sessionStart は fire-and-forget で `getHandleIfLoaded` が空だと additional_context を黙って落とす。beforeSubmitPrompt は待って system_reminder へ載せる。preToolUse の additional_context も次ターンへ載る。同一 hook を三つに置く。beforeSubmitPrompt の `user_message` は `continue: false` のときだけ使われ、本文注入にはならない。subagentStart は `additional_context` を捨てる。`~/.cursor/factory-constitution/.cursor/rules/factory.mdc` は同一正本の overlay。窓への自動 `--add` は last-active を誤るのでしない。
- 残Hの受入は、user hooks を load 済みの Desktop 窓で人が文を送ったチャット。Cmd+Shift+L の新規でも、live reload 済みの既存窓でもよい。次は Desktop hook を踏まないので数えない: goal continuation（この private worker。composer beforeSubmitPrompt を踏まず、exec-daemon の user hooks も起動時スナップショットのまま）、Task/generalPurpose（`bc-` cloud agent。Desktop `hooks.json` を走らせない）、`cursor --chat`（workspaceOpen のみ。x-article 窓が数分おきに再利用される）、未login の `agent -p`、macOS `open` と CLI `--open-url` の `cursor://` prompt（extension の `onUri` が起きない）、osascript keystroke、desktopBridge feature gate。hook 実行の証拠は `cursor-constitution-hook` が Desktop hook ログに `from user config` で出ること（workbench 経路）と、`~/.cursor/factory-hook-state/constitution-delivered/` に stamp が付くこと。新規 private worker が user hooks を読んで tool 側で走らせた場合は、worker ログの `hasUserHooks: true` と stamp でも足りる（workbench の `from user config` は tool だけでは出ない）。
...
- **H:** 実HOMEの`install.sh` / `apply-cursor-config --apply` は済。Wave 6の製品repoは Throughline 0.10.3 まで完了。Macの工場MCPは apply 後の Desktop 別project で tool 列挙、空workspaceの `agent mcp list` で工場6が `ready`。Throughline は `cursor:` session を DB に書いた。残Hは user hooks を load 済みの Desktop 窓で人が文を送り、workbench の `beforeSubmitPrompt` が `cursor-constitution-hook` を走り、stamp が付き、モデルが cap 内案内を受けて `factory.mdc` を Read すること。工場skillは `~/.cursor/skills` から列挙（入れ子 Task でも 4 本は見えた。Claude skill の併記は切断しない）。goal continuation・この worker の tool・Task cloud・`cursor --chat` は数えない。このdotagents窓の workbench は 2026-08-24 13:45 以降 user hooks 13 を load 済み。同じ窓の private worker exec-daemon は 11:46 の `hasUserHooks: false` のままなので、人が送るまで constitution は届かない。
