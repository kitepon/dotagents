...
- [Cursor harness 工場適用](plan_cursor-parent-host.md)（2026-08-24開始。工程正本は当該Markdown。Lattice未適用）。
  [harness用語統一](plan_harness-terminology-refactor-20260824.md)完遂の上に乗る。OS層と既存3harness dirは壊さない。Aiterm `cursor-cli`は消費のみ。Wave 1〜6着地。Mac新規session受入は 2026-08-24 に閉じた（`d4c055fa`）。
