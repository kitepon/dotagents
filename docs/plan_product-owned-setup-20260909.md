...
Windowsへの公式セットアップを実行した。工場設定と更新予約は適用されたが、全体結果は失敗であり、4端末の統合は未完了。2026-09-12、Windows呼出しとCaveat schemaの工場不備を修正し、現行v8の個別検証、Windows関連32試験、最終CIが成功した。詳細と未完条件は[標準呼出しの実測記録](evidence/20260910-windows-public-command-boundary.md)を参照する。AIShellの既存Keychain利用は製品担当の修理・公開待ちで、unai公式更新も製品checkoutの差分で停止している。以下の再開時記録は時点を区別して読む。
