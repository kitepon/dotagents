...
- push／pull requestは変更に関係する検査を、影響する全対応OSで行う。共通実装・CI自身・未分類の変更は全対応OS、OS固有の変更は共通検査と該当OSを選び、混在変更でも対象OSを維持する。検査内容の選択とOSの選択は別に決め、所要時間だけを理由に対応OSを外さない。
- 版番号だけの変更は構造化ファイルの実差分で識別し、版の整合性と配布物の確認を1環境で行う。依存・実行設定・配布内容の変更は版番号変更に含めない。文書だけなら文書検査、実装と文書の混在なら関連試験と文書検査を選ぶ。
