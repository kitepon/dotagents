...
- 段階7完了（2026-09-23）: runbook 8本（reporting・cursor-origin・knowledge-return・git-hygiene・lattice-workflow・jev-computer-use・canon-authoring・factory-ci）を、オーナー裁定と系統的な誤りへの規則だけ残す形に削った（計約6割）。削ったのは、AIが自分で判断・調査できる手順、製品の内部仕様（Latticeの状態判定とrun操作はLattice README、`FACTORY_CI_JOBS`はworkflowとdocs/04）、憲法との重複、日付と経緯、実例。Cursor Originの実測仕様と導入手順は`rag/tools/cursor-origin.md`へ移した。factory-ciには、製品が現行に無いlabelを要求して割り当たらない時は製品repoを直すという境界を足した（ServerManagerの`wsl2`・`linux-native`で実際に起きた）。
- git-hygieneとPLANから「プロジェクト作業はsync-sweep greenから始める」を削った（2026-09-23、オーナー承認。実際には使われておらず、作業前の`git fetch`はAGENTS.mdで足りる）。`bin/sync-sweep.sh`は残す。
- 完了（2026-09-23）。段階1〜7を終えた。段階6のCaveatの注入削減は、Caveatの別開発として進める（別セッションで着手済み）。
