...
- **Lattice既定のscope**: **同一repoへ書込みするworkerを2つ以上同時に走らせる場合**は、オーナーによるLatticeの明示適用を前提に、Lattice run経由（`plan compile`→`run start`）を既定とする。交差判定を親の自前判断で行わない（競合検出と検証済みparallel groupの機構は[合成契約](composition.md)に従う。未検査・staleは競合なしを意味しない）。**別repoへの並列・read-only workerの並列・直列委譲は対象外**。最低安全契約の「writerは専用worktree」は並列時もそのまま適用され、本項はその上に競合判定を重ねるだけで置き換えない。直列化規則そのものの正本は[合成契約](composition.md)であり、本節はその委譲側の帰結だけを持つ。
- **既存runとの照合**: Latticeの明示適用が決まった作業、またはオーナー承認済みの既存Lattice工程を継続する時だけ、着手前に対象repoのrun storeを確認し、active runに属するTODOを二重dispatchしない。中断runの引き継ぎ（resume）・明示退役（abandon＋理由記録）・`run`面と`todo`面の能力差は lattice-workflow runbook に従う（無言の放置・無言の二重起動の両方を禁じる）。
...
5. **characterization 規約**: 現在の実挙動を記録するcharacterization任務に限り、期待を実挙動へ合わせ、プロダクションを直さない。明白な欠陥に見えても報告へ分離する。仕様テスト・不具合修正の回帰テストは、合意済みの仕様と修正後の期待挙動を基準にする。
...
統括は報告を採用宣言に替えない。対象diff、書込範囲、挙動差、Worker Reportの根拠を確認してaccept/rejectを裁定する。親が関連gateを再実行するのは、独立受入として契約が要求する場合、検証後に対象diff・依存・環境が変わった場合、または失敗・未解決の懸念・証拠不足がある場合だけとする。条件が変わらないgreenの検証は、対象と結果の対応を確認して再利用する。委譲中に見つかった前提誤り・想定外の実挙動・再発防止知識は、必要な計画または正本へ還流する。
...
- skill・agents・委譲契約・スクリプトは**役割名**でモデルを指し、具体名への判断は[docs/02_models.md](02_models.md)の順位表だけが担う。runtimeが具体値を要求する`.codex-sidecar.yml`、`claude/agents/*.md`のfrontmatter、Claude Workflowのper-call引数は公認projectionであり、別の判断正本ではない。
