...
const roots = [];
const CURRENT_PROFILE = process.platform === 'darwin' ? 'mac' : process.platform === 'win32' ? 'windows-native' : 'linux';
...
test('post-update gateは完全一致allowlist以外のunverifiedと全failをblockingにする', async () => {
  ...
  const required = ['caveat', 'throughline', 'spotter', 'lattice', 'markitdown', 'gpt-connector', 'aiterm-mcp', 'codex-sidecar', 'peertable', 'unai', 'jev-ultrafast', ...(profile === 'mac' ? ['agent-desktop'] : []), ...(profile === 'server' ? ['servermanager'] : []), ...(profile === 'windows-native' ? [] : ['claude-code', 'codex-cli'])];
