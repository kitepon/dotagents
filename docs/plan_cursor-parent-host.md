...
- 既存Cursor sessionの見た目（apply前のMCP catalog等）は受入に数えない。`hooks.json` は Desktop **workbench** が live reload する（この窓は 13 user hooks を load 済み）。goal continuation は composer の beforeSubmitPrompt を踏まない。**この窓（private worker / background composer、`bc-7face712`）への人間プロンプトも beforeSubmitPrompt を踏まない**（2026-08-24 18:49 JST 実測。stamp 空、当該窓の hook ログは 09:27Z の team refresh 以降更新なし。`from user config` ゼロ）。live reload 済みでも、background composer の follow-up は workbench の `executeHookForStep` を通らない。private worker の exec-daemon は起動時に `~/.cursor/hooks.json` を一度だけ読む（`hasUserHooks`）。この会話の worker は hook 導入前（11:46）に `hasUserHooks: false` で凍結しており、tool の preToolUse も factory hook を走らせない。残Hは Cmd+Shift+L の新規ローカル Agent チャット（新しい worker が今の `hooks.json` を読む）。
...
- 残Hの受入は **Cmd+Shift+L の新規ローカル Agent チャット**。この private worker 窓（`bc-7face712`）は live reload 済みでも、goal continuation も人間の follow-up も beforeSubmitPrompt を踏まない。次も数えない: Task/generalPurpose（`bc-` cloud agent）、`cursor --chat`（workspaceOpen のみ。x-article 窓が数分おきに再利用される）、未login の `agent -p`、macOS `open` と CLI `--open-url` の `cursor://` prompt、osascript keystroke、desktopBridge feature gate。hook 実行の証拠は `cursor-constitution-hook` が Desktop hook ログに `from user config` で出ることと、`~/.cursor/factory-hook-state/constitution-delivered/` に stamp が付くこと。新規 worker が user hooks を読んで tool 側で走らせた場合は、worker ログの `hasUserHooks: true` と stamp でも足りる。
...
- **H:** 実HOMEの`install.sh` / `apply-cursor-config --apply` は済。Wave 6の製品repoは Throughline 0.10.3 まで完了。Macの工場MCPは apply 後の Desktop 別project で tool 列挙、空workspaceの `agent mcp list` で工場6が `ready`。Throughline は `cursor:` session を DB に書いた。残Hは Cmd+Shift+L の新規ローカル Agent チャットで `cursor-constitution-hook` が走り、stamp が付き、モデルが cap 内案内を受けて `factory.mdc` を Read すること。この private worker 窓への人間プロンプト（2026-08-24 18:49 JST）では stamp も `from user config` も付かなかった。工場skillは `~/.cursor/skills` から列挙できた。
