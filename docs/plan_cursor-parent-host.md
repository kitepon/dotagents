...
- 既存Cursor sessionの見た目（apply前のMCP catalog等）は受入に数えない。`hooks.json` は Desktop が live reload する。goal continuation は beforeSubmitPrompt を踏まない。reload済み窓でも、人が送った次の文は hook を踏む。
...
- 残Hの受入は、user hooks を load 済みの Desktop 窓で人が文を送ったチャット。Cmd+Shift+L の新規でも、live reload 済みの既存窓でもよい。次は Desktop hook を踏まないので数えない: goal continuation（private worker。beforeSubmitPrompt / preToolUse がログに出ない）、Task/generalPurpose（`bc-` cloud agent。Desktop `hooks.json` を走らせない）、`cursor --chat`（workspaceOpen のみ。x-article 窓が数分おきに再利用される）、未login の `agent -p`、macOS `open` と CLI `--open-url` の `cursor://` prompt（extension の `onUri` が起きない）、osascript keystroke、desktopBridge feature gate。hook 実行の証拠は `cursor-constitution-hook` が Desktop hook ログに `from user config` で出ることと、`~/.cursor/factory-hook-state/constitution-delivered/` に stamp が付くこと。
...
- **H:** 実HOMEの`install.sh` / `apply-cursor-config --apply` は済。Wave 6の製品repoは Throughline 0.10.3 まで完了。Macの工場MCPは apply 後の Desktop 別project で tool 列挙、空workspaceの `agent mcp list` で工場6が `ready`。Throughline は `cursor:` session を DB に書いた。残Hは user hooks を load 済みの Desktop 窓で人が文を送り、`cursor-constitution-hook` が beforeSubmitPrompt を走り、stamp が付き、モデルが cap 内案内を受けて `factory.mdc` を Read すること。工場skillは `~/.cursor/skills` から列挙（入れ子 Task でも 4 本は見えた。Claude skill の併記は切断しない）。goal continuation・Task cloud・`cursor --chat` は数えない。このdotagents窓は 2026-08-24 13:45 以降 user hooks 13 を load 済みなので、次の人間プロンプトで足りる。
