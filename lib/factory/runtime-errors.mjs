...
export function validateAcknowledgementBundleV9(value, expectedReportId) {
  return validateVersionedAcknowledgementBundle(value, expectedReportId, '9.0');
}
...
export async function acknowledgeRuntimeErrorsV9(bundle, options = {}) {
  validateAcknowledgementBundleV9(bundle, bundle?.report_id);
  return dispatchAcknowledgements(bundle, options);
}
...
function parseAiterm(value) {
  ...
  if (!['aiterm-mcp.runtime-errors.v1', 'aiterm-mcp.runtime-errors.v2'].includes(snapshot.schema_version)) fail('aiterm_schema');
  ...
  return recordsProjection('aiterm-mcp', high,
    projectSequencedRecords('aiterm-mcp', snapshot.records, high, nulFingerprint,
      { legacyFirstVersion: snapshot.schema_version === 'aiterm-mcp.runtime-errors.v1' }));
}
...
function parseCodexSidecar(value) {
  ...
  if (!['2', '3'].includes(snapshot.schema_version)) fail('codex_sidecar_schema');
  ...
  return recordsProjection('codex-sidecar', high,
    projectSequencedRecords('codex-sidecar', snapshot.records, high, nulFingerprint,
      { recordHasProduct: false, legacyFirstVersion: snapshot.schema_version === '2' }));
}
...
function projectSequencedRecords(product, records, high, fingerprintFn, { recordHasProduct = true, legacyFirstVersion = false } = {}) {
  const projected = records.map((record, index) => {
    ...
    if (legacyFirstVersion && record.occurrence_count > 1) delete common.product_version;
...
function projectNativeRecord(product, record, index) {
  ...
  exact(record, ['error_code', 'component', 'status', 'severity', 'fingerprint', 'message_template',
    'occurrence_count', 'first_seen', 'last_seen', 'state_schema_version',
    ...('product_version' in record ? ['product_version'] : [])], path);
  const common = validateCommonRecord(record, path);
...
function validateCommonRecord(record, path) {
  ...
  return {
    ...
    ...('product_version' in record && record.product_version !== 'unknown' ? { product_version: record.product_version } : {}),
