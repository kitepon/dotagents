...
export function codexNativeSpawnRequest(input) {
  exact(input, ["model", "reasoning_effort", "task_name", "task"], "codex native spawn input");
  routingField(input.model, "codex native spawn input.model", 256);
  routingField(input.reasoning_effort, "codex native spawn input.reasoning_effort", 64);
  identifier(input.task_name, "codex native spawn input.task_name");
  nativeTask(input.task, "codex native spawn input.task");
  return codexNativeRequest("spawn", {
    model: input.model, reasoning_effort: input.reasoning_effort, fork_turns: "none",
    message: input.task, task_name: input.task_name,
  });
}

export function codexNativeFollowupRequest(input) {
  exact(input, ["agent_path", "task"], "codex native followup input");
