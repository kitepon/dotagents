...
export function throughlineDiagnostic(diagnostic) {
  ...
  if (diagnostic.schema !== 'throughline.native_factory_diagnostics.v1' || !semver(diagnostic.version) || !nativeStatusV1(diagnostic.overall.status) || !(typeof diagnostic.databaseSchema.schema === 'string' && /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(diagnostic.databaseSchema.schema)) || !nativeStatusV1(diagnostic.databaseSchema.status) || diagnostic.databaseSchema.reason !== canonicalReason(diagnostic.databaseSchema.status) || ![diagnostic.databaseSchema.databaseSchemaVersion].every((value) => value === null || (Number.isSafeInteger(value) && value >= 0)) || diagnostic.hooks.scope !== 'codex' || !nativeStatusV1(diagnostic.hooks.status) || diagnostic.hooks.reason !== canonicalReason(diagnostic.hooks.status) || !Object.values(diagnostic.hooks.events).every(nativeStatusV1) || diagnostic.hooks.status !== aggregateV1(Object.values(diagnostic.hooks.events)) || diagnostic.connectors.codex.status !== diagnostic.hooks.status || diagnostic.overall.status !== aggregateV1([diagnostic.databaseSchema.status, diagnostic.hooks.status, diagnostic.readiness.capture.status, diagnostic.readiness.restore.status, diagnostic.readiness.handoff.status]) || !publicPayload(diagnostic)) throw new Error('native_diagnostics_schema');
...
export function spotterDiagnostic(diagnostic) {
...
function aitermDiagnostic(diagnostic) {
  ...
  const vendorKeys = Object.hasOwn(diagnostic.vendor_dependencies, 'cursor')
    ? ['claude', 'codex', 'grok', 'cursor']
    : Object.hasOwn(diagnostic.vendor_dependencies, 'claude')
      ? ['claude', 'codex', 'grok']
      : ['codex', 'grok'];
