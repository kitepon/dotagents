...
- **製品の不備を工場で補完することは禁止**。dotagentsが製品の導入・設定・更新に使うのは、製品の正規入口と公開結果だけとする。製品に必要な設定・hook・MCP登録・移行・OS差の吸収は製品側が所有する。正規入口の不足や失敗を、工場側の直接編集・内部手順の組立て・独自wrapperで補うと単体の不具合を隠すため、それらは製品側の修理対象とする。失敗・未対応はそのまま可視化し、成功として扱わない。dotagents自身の規範・設定・hook・配布・更新予約・工場報告はdotagentsが所有する。
