...
export function assertWindowsNativeProductSmoke(report, runtimeArch = process.arch) {
  ...
  if (report.schema_version !== '9.0' || report.host_profile !== 'windows-native') throw new Error('report is not Windows native wire v9');
  ...
  if ('observer' in products) throw new Error('observer must be absent from the wire v9 product set');
  const expected = [...CURRENT_WIRE_PRODUCT_IDS].sort();
  const actual = Object.keys(products).sort();
  if (actual.length !== expected.length || actual.some((id, index) => id !== expected[index])) throw new Error('wire v9 product set is incomplete or contains unknown products');
