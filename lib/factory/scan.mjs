...
function latticeNative(result, diagnostic, observedAt) {
  ...
  if (!semver(diagnostic?.version) || !['ok', 'failed'].includes(diagnostic.overall)) return invalid();
...
function aishellNative(result, diagnostic, observedAt) {
  ...
  // 報告に使う公開結果だけを読み、未使用の内部情報を検査しない。
  if (!result.ok || !semver(diagnostic?.product?.version) || typeof diagnostic.ready !== 'boolean') {
    product.checks.push({
      check_id: 'native_diagnostics', status: 'unverified', reason_code: 'native_schema_invalid',
    });
    return product;
