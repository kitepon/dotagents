...
固定Recipe `adversarial-audit`／`bulk-curation` のPhase・入出力schema・reducer・gate・失敗条件の正本は[固定Recipe契約](../../../shared/orchestrate/recipes.md)。本節はCursor実行入口だけを所有する。

- **fan-out**: 各視点・各指摘・各対象を1子1任務として`Task`へ出す。read-only段は並列してよい。同一repoへ書込む対象が2つ以上ありLatticeが選択されていなければ直列（正本は[合成契約](../../../shared/orchestrate/composition.md)）。
